...
class MySpider(CrawlSpider):
    ...
    rules = [
        Rule(
            LinkExtractor( allow_domains='www.starrocks.io', deny=('patterToBeExcluded'), unique=('Yes')), 
            callback='parse_my_url', # method that will be called for each request
            follow=True),
        # crawl external links but don't follow them
        Rule(
            LinkExtractor( allow=('docs.starrocks.io'),deny=("patterToBeExcluded"),unique=('Yes')),
